...
void plotPi0_pPb2013(const int iOption = 2,        // default plotting choice of HBT correlations
		     ...
		     const float lambdaHBT = 1.0,           // lambda value in HBT pairs correlation function
                     ...
                     const int lowClusterPairCrystalSumLimit = 3,       // lower limit on the sum of the crystals in a cluster pair
                     const int highClusterPairCrystalSumLimit = 3,      // upper limit on the sum of the crystals in a cluster pair
  ...
  if(abs(iOption) >= 10) { // using hTree to produce figures
      ...
      for (Long64_t i=0; i<entriesToProcess; i++) {
        ...
        for(int jMeson=0; jMeson< nMesonSize; jMeson++) {
          ...
          if(pdgIdMeson[jMeson] == pdgIdMesonChoice && daughtersFromMeson[jMeson] == 2 && fabs(etaMeson[jMeson]) <= etaHighLimit &&
             fabs(etaMeson[jMeson]) >= etaLowLimit && ptMeson[jMeson] > 0.500 && ptMeson[jMeson] < 12.0) {
    ...
    if(iOption == 31 || iOption == -31 || iOption == 32 || iOption == -32 || iOption == 33 || iOption == -33 || iOption == 53 ||
      ...
      int nPtBins = 4;
      float lowPtBinLimit[4] =  {1.0, 1.5, 2.0, 2.5};
      float highPtBinLimit[4] = {1.5, 2.0, 2.5, 3.0};

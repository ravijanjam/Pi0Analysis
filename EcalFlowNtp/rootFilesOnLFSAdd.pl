...
my $storePath = "/lio/lfs/cms/store" . $storeDirectory;
...
#
    `lio_ls /lio/lfs/cms/store/user/maguirc/hiRun2013pPb/ | grep 1-100 > storeFilesList.txt`;
   #`lio_ls $storePath | grep 24Ju |  > storeFilesList.txt`;
    ...
    my $numberLastFile = 500;  # maximum number of files to select

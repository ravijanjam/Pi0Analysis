...
// Last revision: March 6, 2014 at 16:55 CST
//
...
void pi0MassFit(const int iMultOption = 0,
  ...
  //
  // This logic is hard-coded for only the lowest 4 pT bins
  //
  ...
  for(int nn=0; nn<iPad; nn++)
	...
	if(nn==0){
	  ...
	  tex->SetLineWidth(2);
	  tex->Draw();
	  TLatex *tex2 = new TLatex(0.08, 0.10*histogramMaximumValue, Form("%5.1f < p_{T} < %5.1f (GeV/c)",_ptMin[nn],_ptMax[nn]));
	  tex2->SetTextFont(42);
	...
	if(nn>0){
	  TLatex *tex = new TLatex(0.014, 0.6*histogramMaximumValue ,Form("%5.1f < p_{T} < %5.1f (GeV/c)",_ptMin[nn],_ptMax[nn]));
	  tex->SetTextFont(42);
	  tex->SetTextSize(0.06974556);
	  tex->SetLineWidth(2);
	  tex->Draw();
      ...
      if(nn >=4 && nn<=7)
	{
	  ...
	  cout<<"***********************************"<<endl;
	  cout<<Form("%5.1f < p_{T} < %5.1f (GeV/c)",_ptMin[nn-4],_ptMax[nn-4])<<endl;
	  cout<<"------------"<<nn-4<<"-------------"<<endl;
  ...
  for(int nn=0; nn<4; nn++)
    {
        cout<<Form("%5.1f < p_{T} < %5.1f (GeV/c)",_ptMin[nn],_ptMax[nn])<<'\t'<<"v2 Signal = "<<v2Sig[nn]<<'\t'<<"v2 signal error = "<<v2SigErr[nn]<<endl;
    }
    cout<<"***********************************"<<endl;
}

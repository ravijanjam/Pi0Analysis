...
my $storeDirectory = "/user/maguire/hiRun2013pp/";
...
#
# Generate the list of files in this store area (assume that the files the proper key in their names)
#

print "\n Generating storeFilesList.txt\n";

`ls -al $storePath | grep "pp_MinBiasTimingCutWeighted2D_OneToFourCrystals_3Feb" > storeFilesList.txt`;
...
print "\n Examining storeFilesList.txt\n";

my $numberFirstFile = 1;
my $numberLastFile = 20;
...
if($totalFilesAdded > 0) {
    if($totalFilesAdded <= $fileListSplit) {
	print "\n\n Renaming pPbAllInputSets.root from pPbFileSum_1.root\n";
	`mv pPbFileSum_1.root pPbAllInputSets.root`;
    }
    else {

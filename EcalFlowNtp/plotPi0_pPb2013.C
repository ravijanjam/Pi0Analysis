...
void plotPi0_pPb2013(const int iOption = 33,        // default plotting of two pT bins of background-subtracted pair mass spectra
		     ...
		     const float widthForEfficiency = 0.030,  // for the 331 efficiency option, the accepted reco are within +/- widthForEfficiency (GeV/c^2) of the 0.134976 MeV pi0 value
  ...
  const bool noReCalFromEDM = false;
	...
	*/
	...
	for(int kPt=0; kPt<4; kPt++) {
	  ...
	  if(selectSameParentClusters) {
	    ...
	    if(!noReCalFromEDM) {
	      ...
	      int maxBin = sameEventMassHistogramsPtBin[kPt]->GetMaximumBin();
	      double massValueMaximum = sameEventMassHistogramsPtBin[kPt]->GetBinCenter(maxBin);

	      double startMass = massValueMaximum - 0.030;  // 30 MeV below the maximum
	      if(startMass < 0.03)
		startMass = 0.03;

 	      double endMass = massValueMaximum + 0.030;  // 30 MeV above the maximum

	      sameEventMassHistogramsPtBin[kPt]->Fit("gaus", "", "", startMass, endMass);
	    }
	    ...
	    cout << "\n  Centroid = " << centroid;
	    cout << ",  Width = " << width;
	    double trueSum = 0.0;    // true sum is defined as the yield within +/- 30 MeV of the centroid, independent of the fit sigma
	    double trueMassLow = centroid - 0.03;
	    double trueMassHigh = centroid + 0.03;
	    ...
	    for(unsigned int kBin=0; kBin<=75; kBin++) {
	      ...
	      if(massValue >= trueMassLow && massValue <= trueMassHigh) {
		trueSum += sameEventMassHistogramsPtBin[kPt]->GetBinContent(kBin);
	      }
	    ...
	    char *peakLabel1 = new char[200];
	    sprintf(peakLabel1, "Fit: centroid = %5.1f MeV/c^{2}", 1000.*centroid);
	    ...
	    char *peakLabel2 = new char[200];
	    sprintf(peakLabel2, "Fit: #sigma = %4.1f MeV/c^{2}", 1000.*width);
	    ...
	    char *yieldLabel2 = new char[200];
	    sprintf(yieldLabel2, "True sum = %d #pm %d", int(trueSum), int(sqrt(trueSum))+1);
	    TLatex *yieldText2 = new TLatex(0.01, (0.8 - 2.0*deltaY)*maxHistogramValue2, yieldLabel2);
	    yieldText2->SetTextColor(kBlue);
	    yieldText2->SetTextSize(0.05);
	    yieldText2->Draw();
	  ...
	  if(kPt == 0) {
		...
		runLabel6 = new TLatex(0.02, 0.58*maxHistogramValue1, "Only same parent photon pairs are used, no check for #pi^{0} parent");
	      ...
	      if(noReCalFromEDM) {
		TLatex *runLabel7 = new TLatex(0.02, 0.51*maxHistogramValue1, "ECAL clusters NOT recalibrated (do not believe p_{T} values !)");
		runLabel7->SetTextColor(kRed);
		...
		TLatex *plotLabel = new TLatex(0.02, 0.51*maxHistogramValue1, plotLabelText);
	...
	if(iOption == 31 || iOption == 33) {
	  ...
	  if(iOption == 33) {
		...
		double trueSum = 0.0;
		double trueMassLow = centroidFit - 0.03;
		double trueMassHigh = centroidFit + 0.03;
		...
		for(int massBin=1; massBin<200; massBin++) {
		  ...
		  if(massValue >= trueMassLow && massValue <= trueMassHigh) {
		    trueSum += sameEventMassHistogramsPtBin[0]->GetBinContent(massBin);
		  }
		  if(massValue >= lowCountLimit && massValue <= highCountLimit) {
		...
		char *yieldText = new char[200];
		sprintf(yieldText, "Peak yield in %d#sigma interval = %d #pm %d, FG Sum in #pm 30 MeV = %d #pm %d",
			int(sigFactor), int(summedYield), int(summedYieldError), int(trueSum), int(sqrt(trueSum)));
		...
		for(int massBin=1; massBin<200; massBin++) {
		  ...
		  if(massValue >= trueMassLow && massValue <= trueMassHigh) {
		    trueSum += sameEventMassHistogramsPtBin[1]->GetBinContent(massBin);
		  }
		  if(massValue >= lowCountLimit && massValue <= highCountLimit) {
	...
	for(int kPt=0; kPt<4; kPt++) {
      ...
      char *graphOutputFileName = new char[200];
      sprintf(graphOutputFileName, "pi0MassHighPt_Q%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_Date%d.pdf", pTQuartile, lowMultiplicity, highMultiplicity, clustS25Cut, date);
      c1->Print(graphOutputFileName);  // PDF file
      sprintf(graphOutputFileName, "pi0MassHighPt_Q%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_Date%d.root", pTQuartile, lowMultiplicity, highMultiplicity, clustS25Cut, date);
      c1->Print(graphOutputFileName);  // ROOT file
  ...
  if(iOption == -6 || iOption == 6 || iOption == 7 || iOption == -7) { // parameterized bacgkround fitting for the pair mass spectrum from the EDM output
   ...
   sprintf(histogramName, "%spi0MassMixedEventPtBin%d", multText, kPtBinEDM);
    ...
    char *graphOutputFileName = new char[200];
    if(iOption == 6) {
      sprintf(graphOutputFileName, "pi0MassFitLinearCompare_PtBin%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_%d.pdf", kPtBinEDM, lowMultiplicity, highMultiplicity, clustS25Cut, date);
      c1->Print(graphOutputFileName);  // PDF file
      sprintf(graphOutputFileName, "pi0MassFitLinearCompare_PtBin%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_%d.root", kPtBinEDM, lowMultiplicity, highMultiplicity,clustS25Cut,  date);
      c1->Print(graphOutputFileName);  // ROOT file
    }
    if(iOption == 7) {
      sprintf(graphOutputFileName, "pi0MassFitQuadraticCompare_PtBin%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_%d.pdf", kPtBinEDM, lowMultiplicity, highMultiplicity, clustS25Cut, date);
      c1->Print(graphOutputFileName);  // PDF file
      sprintf(graphOutputFileName, "pi0MassFitQuadraticCompare_PtBin%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_%d.root", kPtBinEDM, lowMultiplicity, highMultiplicity, clustS25Cut,date);
      c1->Print(graphOutputFileName);  // ROOT file
    }
    if(iOption == -7) {
      sprintf(graphOutputFileName, "pi0MassFitSquareRootCompare_PtBin%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_%d.pdf", kPtBinEDM, lowMultiplicity, highMultiplicity, clustS25Cut, date);
      c1->Print(graphOutputFileName);  // PDF file
      sprintf(graphOutputFileName, "pi0MassFitSquareRootCompare_PtBin%d_489Files_Multiplicity%d-%d_S25Cut-%3.1f_%d.root", kPtBinEDM, lowMultiplicity, highMultiplicity, clustS25Cut, date);
      c1->Print(graphOutputFileName);  // ROOT file
    }	
    return;
  ...
  if(iOption == 9 || iOption == -9) { // plot EDM result, from Monika's code
    ...
    char *graphOutputFileName = new char[200];
    sprintf(graphOutputFileName, "pi0MassHighPt_PtBin%d_489Files_Multiplicity%d-%d_%d.pdf", kPtBinEDM, lowMultiplicity, highMultiplicity, date);
    c1->Print(graphOutputFileName);  // PDF file
    sprintf(graphOutputFileName, "pi0MassHighPt_PtBin%d_489Files_Multiplicity%d-%d_%d.root", kPtBinEDM, lowMultiplicity, highMultiplicity, date);
    c1->Print(graphOutputFileName);  // ROOT file

...
void pi0InvMassPbPb_120Ntrack150_AllptBins()
{
  ...
  TString _trueHistogramFile = "inputFile.root";
  ...
  TString _filePeakRegion_120M150 = "PeakRegion_120Mult150.txt";
  TString _fileSideBand_120M150 = "SideBand_120Mult150.txt";
  ...
  for(long iptBin=0; iptBin<8; iptBin++)
	...
	if(!_trueHisto[iptBin]) {
	  cerr << "\n For iptBin = " << iptBin << " unable to find " << _trueHistoName[iptBin] << endl;
	  return;
	}

	_mixedHisto[iptBin] = (TH1D*)_ftrueHistogram->Get(_mixedHistoName[iptBin]);
	if(!_mixedHisto[iptBin]) {
	  cerr << "\n For iptBin = " << iptBin << " unable to find " << _mixedHistoName[iptBin] << endl;
	  return;
	}
    }
  ...
  for(int nn=0; nn<iPad; nn++)
    {
      ...
      cout << "\n At nn = " << nn << endl;
	    ...
	    double sameEventCount = 0.0;
            ...
            if(MixedEventCount > 0.0 && sameEventCount > 0.0 ) { // background subtraction for unpartitioned yields
                ...
                if(nn==0){
                    ...
                    tex->SetLineWidth(2);
                    tex->Draw();
                    TLatex *tex2 = new TLatex(0.08,6000,Form("%5.1f < p_{T} < %5.1f (GeV/c)",pTbins[nn],pTbins[nn+1]));
                    tex2->SetTextFont(42);
		...
		//
                // subtractedYieldErrors = sqrt(sumSubtractedError*sumSubtractedError + mixedEventYieldError*mixedEventYieldError);
                //
                ...
                // float S_B = sumSubtractedYield/mixedEventYield; // variable not used
                ...
                // float Db0 = mixedEventYieldError;  // variable not used

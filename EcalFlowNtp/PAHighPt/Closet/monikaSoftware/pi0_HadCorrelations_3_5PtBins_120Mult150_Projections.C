...
void pi0_HadCorrelations_3_5PtBins_120Mult150_Projections()
{
    ...
    ofstream myfile;
    myfile.open ("PeakRegion_120Mult150.txt");

    TString _pPbData = "PbPb_HIMinBiasUPC_PeakRegion095_175_NoCuts_YesCorrections_3_5PtBins.root";
 
    TFile *_fpPbData = new TFile(_pPbData, "r");
    TH1F * _hEvent_pPb;
    double _nEvents_pPb;
    TH2D *_hSignal_pPb[8];
    TH2D *_hBackground_pPb[8];
    
    TString _EventBaseName = "EcalFlowNtpMult100/ChargedParticles/tHPTracks_";
    TString _SignalHistoName = "EcalFlowNtpMult100/pi0HadCorrFunc/hSignalPtBin";
    TString _BkgHistoName = "EcalFlowNtpMult100/pi0HadCorrFunc/hBackgroundPtBin";
    
    TString _SignalHistoName1, _BkgHistoName1;
    ...
    _hEvent_pPb = (TH1F*)_fpPbData->Get(_EventBaseName);
    _nEvents_pPb = _hEvent_pPb->Integral();
    
    for(long iCount=0; iCount<8; ++iCount)
    {
        _SignalHistoName1 = _SignalHistoName + iCount;
        _BkgHistoName1 = _BkgHistoName + iCount;
        cout<<iCount<<'\t'<<"Signal name = "<<_SignalHistoName1<<endl;
        cout<<iCount<<'\t'<<"Background name = "<<_BkgHistoName1<<endl;
    ...
    cout<<"Now drawing the projections..."<<endl;
    float etaMin = 2.0;
    float etaMax = 3.6;
    ...
    for(long iCount=0; iCount<8; ++iCount)
    {
       ...
       /* _FourierDecompose[iCount] = new TF1(_Name5, v2Fit, -1.4, 4.88, 4);
        ...
        _Parameter3[iCount] = _FourierDecompose[iCount]->GetParameter(3);
        _Par3Error[iCount] = _FourierDecompose[iCount]->GetParError(3);
    
        _Ratio[iCount] = _Parameter2[iCount]/_Parameter0[iCount];
        
        _Ratio[iCount] /= 0.08609;
        
        _Error[iCount] = _Ratio[iCount]*(sqrt((_Par2Error[iCount]/_Parameter2[iCount])*(_Par2Error[iCount]/_Parameter2[iCount]) + (_Par0Error[iCount]/_Parameter0[iCount])*(_Par0Error[iCount]/_Parameter0[iCount]) + (0.0001364/0.08609)*(0.0001364/0.08609)));

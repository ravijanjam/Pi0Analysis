...
//
void pi0_HadCorrelations_Projections(const int iMultOption = 0,
  ...
  //                  this could be either a PbPb, or a pPb, or a (future) pPb simulation ROOT file from the EDM
  //
  ...
  for(long iCount=0; iCount<nPtBins; ++iCount) {
    ...
    _pi0HadCorrProj[iCount]->SetMaximum(maximumHistogramValue + 0.50*(maximumHistogramValue - minimumHistogramValue));
    ...
    if(peakRegionFile) {
      sprintf(ptRangeText, "Peak: %4.1f < p_{T} < %4.1f (GeV/c)", _ptMin[iCount], _ptMax[iCount]);
    }
    legend->AddEntry(_Name5, ptRangeText, "l");
    legend->Draw();

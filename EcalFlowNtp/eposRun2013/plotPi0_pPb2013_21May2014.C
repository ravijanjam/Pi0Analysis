...
void plotPi0_pPb2013(const int iOption = 2,        // default plotting choice of HBT correlations
                     ...
                     const bool useConversionSuppression = true, // suppress converted photons for MC input
		     ...
		     const bool useTrackEfficiencyTable = true,     // using external efficiency root file for charged track losses
		     ...
		     const float seedTimeCut = 0.0,           // fixed-size cut on time of the seed crystal (0 means no cut)
                     ...
                     const bool useClusterPairCrystalSumLimit = true,
  ...
  if(useHijing && useEPOS) {
    cerr << "\n Cannot set simulataneously useHijing as true and set useEPOS as true as true" << endl;
    return;
  }
  ...
  if(useConversionSuppression && (selectConversionOnly || selectNonConversionOnly)) {
    cerr << "\n Cannot set  useConversionSuppression  as true  and then have either  selectConversionOnly  or  selectNonConversionOnly  as true" << endl;
    return;
  } // safety check

  const bool testConversion = useConversionSuppression || selectConversionOnly || selectNonConversionOnly;
  ...
  float lowLimitPi0Mass = 0.100;  // true value at 0.095, side band at 0.210
  float highLimitPi0Mass = 0.170; // true value at 0.175, side band at 0.290
  ...
  if(abs(iOption) >= 10) { // using hTree to produce figures
	...
	bool convertedClusterLast[2000];
	...
	for(long kPt=0; kPt<nPtBinsComplete; kPt++) {
	  ...
	  if(iOption == 337) {
		...
		recoText2->Draw();
	      ...
	      if(selectConversionOnly) {
		TLatex *recoText2 = new TLatex(0.62, minimumHistogramValue + 0.53*(maximumHistogramValue1-minimumHistogramValue), "Conversion clusters only for #pi^{0} RECO");
		recoText2->SetTextColor(2);
		recoText2->SetTextSize(0.042);
		recoText2->Draw();
	      }
	      ...
	      if(selectNonConversionOnly) {
		TLatex *recoText2 = new TLatex(0.62, minimumHistogramValue + 0.53*(maximumHistogramValue1-minimumHistogramValue), "Non-conversion clusters only for #pi^{0} RECO");
		recoText2->SetTextColor(2);
		recoText2->SetTextSize(0.042);
		recoText2->Draw();
	      }
